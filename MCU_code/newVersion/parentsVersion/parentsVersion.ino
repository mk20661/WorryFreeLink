...
#define LED_working_2 6
#define LED_working_3 7

#define LED_free_to_connect_1 8
#define LED_free_to_connect_2 9
#define LED_free_to_connect_3 10
...
std::vector<int> kidLEDPins2 = {LED_working_2,LED_free_to_connect_2};
std::vector<int> kidLEDPins3 = {LED_working_3,LED_free_to_connect_3};
std::vector<String> attentionAudio = {exercising,social,others};
...
void setup() {
   ...
   for (int pin : {LED_Older_1, LED_Older_2, LED_Older_3,
                  LED_working_1, LED_working_2, LED_working_3,
                  LED_free_to_connect_1, LED_free_to_connect_2, LED_free_to_connect_3}) {
        pinMode(pin, OUTPUT);
...
void selectStatus(void *parameter) {
  ...
  while (1) {
    ...
    currentOlederLEDIndex_case1 = (currentOlederLEDIndex_case1 + 1) % 3;
...
void mqttsendmessage() {
  ...
  const char* topic = subscribeTopic1;
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if (String(topic) == subscribeTopic2) {
        ...
        int ledIndex = receivedMessage.toInt();
        if (ledIndex >= 1 && ledIndex <= 3) {
            for (int pin : kidLEDPins3) {
                digitalWrite(pin, LOW);
            }
            int ledPin = kidLEDPins3[ledIndex - 1]; 
            Serial.print("Turning ON kid LED: ");
            Serial.println(ledPin);
            digitalWrite(ledPin, HIGH);
        }
